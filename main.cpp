...
template<typename T>
T** add_col_to_Matrix(T** arr, int row, int &col, int index)
{
    col += 1; // add 1 col
    ...
    for (int i = 0; i < row; i++) // write new row, from old to new matrix
    {
        for (int j = 0; j < col; j++)
        {
            if (j < index) {
...
int main()
{
    ...
    int** arr_1 = add_col_to_Matrix<int>(arr, row, col, index);
    cout << endl << endl;
